...
class BlockBEE extends \Opencart\System\Engine\Controller
{
    public function index(): string
    {
        if ($this->config->get('payment_blockbee_status')) {
          ...
          foreach ($this->config->get('payment_blockbee_cryptocurrencies') as $selected) {
            if (!empty($addressOut = \Opencart\Extension\BlockBEE\System\Library\BlockBEEHelper::get_address_out($selected, $apiKey, $callbackUrl))){
              $da_cry += [
                $selected=>$selected,
              ];
            }
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if ($selected) {
                ...
                if ($blockchain_fee) {
                    $blockbeeFee += floatval(\Opencart\Extension\BlockBEE\System\Library\BlockBEEHelper::get_estimate($this->session->data['blockbee_selected'])->$currency);
                }
            }
    ...
    public function pay()
    {
        ...
        $params = [
            'module_path' => \Opencart\Extension\BlockBEE\System\Library\BlockBEEHelper::base_url() . '/extension/blockbee/catalog/view/image/',
            ...
            'branding_logo' => \Opencart\Extension\BlockBEE\System\Library\BlockBEEHelper::base_url() . '/extension/blockbee/catalog/view/image/payment.png',
        ...
        foreach ($orders as $order) {
            ...
            if (!empty($metaData['blockbee_last_price_update'])) {
                ...
                if ($order_timeout !== 0 && (strtotime($order['date_added']) + $order_timeout) <= time() && $already_paid <= 0 && (int)$metaData['blockbee_cancelled'] === 0) {
                    ...
                    $this->model_extension_blockbee_payment_blockbee->updatePaymentData($order_id, 'blockbee_cancelled', '1');
                }
            }
        }
